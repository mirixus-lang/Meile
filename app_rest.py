...
DATA_FILE = Path(__file__).resolve().parent / "milestones_rest.json"
...
def projects_to_payload(projects: list[dict]) -> dict:
    """Convert in-memory project sheets to the JSON structure used on disk/GitHub."""
    return {
        "projects": [
            {
                "project": {
...
def projects_from_payload(data) -> list[dict]:
    """Load current and legacy JSON payload shapes into project sheets."""
...
def github_secret(name: str, fallback: str | None = None) -> str | None:
    """Read a Streamlit secret without failing when no secrets file exists locally."""
    try:
...
def github_config() -> dict | None:
    """Return GitHub Contents API settings, or None when REST storage is disabled."""
    token = github_secret("GITHUB_TOKEN")
    owner = github_secret("GITHUB_OWNER")
    ...
    return {
        ...
        "path": github_secret("GITHUB_DATA_PATH", DATA_FILE.name),
    }


def github_file_url(config: dict) -> str:
    path = urllib.parse.quote(config["path"], safe="/")
    return (
        f"{GITHUB_API_ROOT}/repos/{config['owner']}/{config['repo']}"
        f"/contents/{path}"
    )
...
def github_request(
    ...
    """Execute one GitHub API request and return the decoded JSON body."""
    body = None
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "meilensteinplaner-streamlit",
        },
    )
...
def read_payload_from_github(config: dict) -> tuple[dict | list | None, str | None]:
    """Read the configured GitHub JSON file and return payload plus file SHA."""
    url = f"{github_file_url(config)}?ref={urllib.parse.quote(config['branch'])}"
    try:
        response = github_request(config, "GET", url)
...
def write_payload_to_github(config: dict, payload: dict) -> None:
    """Commit the project payload to GitHub through the Contents API."""
    _, sha = read_payload_from_github(config)
    content = json.dumps(payload, ensure_ascii=False, indent=2)
    request_payload = {
        "message": f"Update {config['path']}",
        "content": base64.b64encode(content.encode("utf-8")).decode("ascii"),
        "branch": config["branch"],
    }
    if sha:
        request_payload["sha"] = sha
    github_request(config, "PUT", github_file_url(config), request_payload)


def save_projects(projects: list[dict]) -> None:
    """Persist all project sheets locally or as a GitHub commit when configured."""
    payload = projects_to_payload(projects)
    config = github_config()
    if config:
        try:
            write_payload_to_github(config, payload)
            return
        except urllib.error.HTTPError as error:
            if error.code == 409:
                st.error(
                    "GitHub-Speichern fehlgeschlagen: Die Datei wurde "
                    "zwischenzeitlich geändert. Bitte neu laden und erneut speichern."
                )
            else:
                st.error(f"GitHub-Speichern fehlgeschlagen: HTTP {error.code}")
            return
        except (OSError, ValueError, KeyError) as error:
            st.error(f"GitHub-Speichern fehlgeschlagen: {error}")
            return
...
def load_projects() -> list[dict]:
    """Load project sheets from GitHub when configured, otherwise locally."""
    config = github_config()
    if config:
        try:
            payload, _ = read_payload_from_github(config)
            return projects_from_payload(payload) if payload is not None else [project_entry()]
        except (urllib.error.HTTPError, OSError, ValueError, KeyError) as error:
            st.warning(
                "GitHub-Daten konnten nicht geladen werden. "
                f"Es wird eine neue Tabelle verwendet. Fehler: {error}"
            )
    ...
    try:
        return projects_from_payload(json.loads(DATA_FILE.read_text(encoding="utf-8")))
